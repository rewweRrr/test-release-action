import { weeekApiConfig as config } from '../utils/config.js'
...
export async function createWeeekTask(body) {
  logDebug(`Creating weeek task with body: ${JSON.stringify(body, null, 2)}`)
  const data = await fetchWeeek('tm/tasks', {
    method: 'POST',
    body: JSON.stringify(body),
  })
...
export async function moveWeeekTaskToColumn(taskId, body) {
  logDebug(`Move weeek task ${taskId} with body: ${JSON.stringify(body, null, 2)}`)
  const data = await fetchWeeek(`tm/tasks/${taskId}/board-column`, {
    method: 'POST',
    body: JSON.stringify(body),
  })
  return data.tag
